...
This algorithm will produce the changes to a projection as illustrated in Figure \ref{fig:manualsequence}. The controlled variable, $V_m$, corresponds to the dark line, and sequential changes to row $m$ of $A$ can be seen to roughly follow a specified position (dot). Changes in the other components happen as a result of the orthonormalisation, but are uncontrolled. 
...
```{r manualsequence, out.width="100%", width=12, height=4, fig.cap="Sequence of projections where contribution of one variable is controlled (dark) is changed using unconstrained orthonormalisation. The dot indicates the chosen values for the controlled variable, $V_m$. It can be seen that the actual axis does not precisely match the chosen position, but it is close."}
...
Changing slice center manually can be accomplished with the new function, `manual_slice()`. This changes the value of the center point ($\mathbf{c}$) of the slice, along a selected variable axis. It slices from the center of the data out to one side, back to the center, out to the opposite side, and then back to the center. <!--The appendix contains a discussion of a possible visual guide for the slice center location.-->
