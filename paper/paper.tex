...
\begin{abstract}
We implement a new version of a manual tour that takes advantage of
existing features in Mathematica to construct functions that interact
with the position of the cursor. We demonstrate the usefulness of this
new tool with an example exploring region boundaries in classification
problems.
\end{abstract}
...
\{\textcolor{blue}Another very useful feature is that the user can keep
the projection fixed while slicing across the orthogonal space and
changing the centre point used for slicing. This can serve to illustrate
how the orthogonal space affects correlations in a slice and how they
may change or disappear in a projection.\}

This paper explains a simpler approach, which is more direct, and
extensible for generating user interaction, and is organized as follows.
The next section describes the new algorithm for manual control, for
both projections and slices. The use of these methods are illustrated to
compare and contrast boundaries constructed by different classifiers.
...
application, and the other that could be used generically for new data.
...
XXX Equation for arbitrary center for slice; and move the ``the
implementation \ldots{}'' into the software section once that is
constructed; and ``potential adjustments for future'' into the section
on desirable controls
