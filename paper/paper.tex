...
Through manual rotation of the view we can get a feeling for where in
the space we primarily predict each of the three species, and we also
get a sense of the difference between the two models. To illustrate this
difference we have rotated the projection view from the RF model to
identify a projection that shows the non-linear, block-type structure
introduced by the model, see Fig. \ref{proj1}. We then export the
corresponding matrix \(A_1\) to look at the same projection for the LDA
model and the original data. Here we notice that while the overall
picture is similar, the linearity of the LDA leads to a big difference
in how two of the three species overlap across the projection (blue vs
green, i.e.~Adelie vs Gentoo penguins). Interestingly that same
projection leads to almost perfect separation of the three species when
looking at the data points, the overlap between the models is driven by
what is happening in the orthogonal space to the plane.
