...
\textcolor{blue}{One might implement such an interactive interface in R, for example using `shiny` [@shinym], with `plotly` [@plotly] drawing. It could be possible with `rgl` [@rgl] but this package is focused on 3D, not the arbitrary $p$-D of tours. The primary tool needed is the tracking of the cursor position in small increments and it is not readily available in R. The `plotly` package is designed to add on to `ggplot2` graphics, not to be a programmable interactive graphics system. It would have been possible with the `cranvas` system [@cranvas] or `rJava` [@rJava] but neither has a graphics toolkit that operates across platforms today. What we have chosen to do is to use the interactivity of Mathematica to experiment, and to implement as an animation in the `tourr` [@tourr] package. There is hope that better interactivity will be available for R soon, perhaps in javascript, which might allow for experimentation to develop new techniques as available in Mathematica.}
